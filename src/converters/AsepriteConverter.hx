...
class AsepriteConverter {
            ...
            for (file in files) {
                ...
                if (!sys.FileSystem.isDirectory(path)) {
                    if (StringTools.endsWith(path, ext)) {
                        ...
                        if (StringTools.endsWith(path, '.character$ext')) {
                            output = output.substr(0, output.length - ".character".length);
                            generateCharacterAseFile(input, output);
                        } else {
    ...
    static function getAsepriteLayerData(aseFilePath : String) {
        ...
        var c : kek.graphics.AsepriteResource.AseFile = haxe.Json.parse(text);
        FileSystem.deleteFile(tmpFile);
        return c.meta.layers;
    }

    static function generateCharacterAseFile(aseFilePath : String, destPath : String) {
        convertAseFile(aseFilePath, '${destPath}/body', { layers: ["Body"], hideLayers: ["Legs", "ItemsFront", "ItemsBehind"]});
        convertAseFile(aseFilePath, '${destPath}/legs', { layers: ["Legs"], hideLayers: ["Body", "ItemsFront", "ItemsBack"]});

        var layers = getAsepriteLayerData(aseFilePath);
        for (layer in layers) {
            if (layer.group == "ItemsFront" && layer.name != null) {
                convertAseFile(aseFilePath, 
                    '${destPath}/item_${layer.name.toLowerCase()}', 
                    { layers : [layer.name, layer.group]});
            }
            if (layer.group == "ItemsBack" && layer.name != null) {
                convertAseFile(aseFilePath, 
                    '${destPath}/item_${layer.name.toLowerCase()}', 
                    { layers : [layer.name, layer.group], hideLayers: [ "ItemsFront" ]});
            }
        }
    }
}
